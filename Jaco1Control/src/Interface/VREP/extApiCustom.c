...
EXTAPI_DLLEXPORT simxInt simxCustomGetTime(simxInt clientID,simxFloat* time,simxInt operationMode)
{
	simxInt returnValue;
	simxUChar* dataPointer;
	...
	/* Then take care of the "remove" operation mode: */
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_null(clientID,simx_customcmd_get_time)); /* _int indicates that the command is identified by the command ID AND an integer value */


	/* Now "execute" the command (i.e. place the command (with its associated command data) into the outbox, and retrieve a command reply from the inbox) */
    dataPointer=_exec_null(clientID,simx_customcmd_get_time,operationMode,0,&returnValue); 

    if ((dataPointer!=0)&&(returnValue==0))
    	time[0]=_readPureDataFloat(dataPointer,0,0); /* pure data is the data that is not used to identify a command */
...
EXTAPI_DLLEXPORT simxInt simxCustomGetDelta(simxInt clientID,simxFloat* delta,simxInt operationMode)
{
	...
	/* Then take care of the "remove" operation mode: */
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_null(clientID,simx_customcmd_get_delta)); /* _int indicates that the command is identified by the command ID AND an integer value */


	/* Now "execute" the command (i.e. place the command (with its associated command data) into the outbox, and retrieve a command reply from the inbox) */
    dataPointer=_exec_null(clientID,simx_customcmd_get_delta,operationMode,0,&returnValue); 
    if ((dataPointer!=0)&&(returnValue==0))
    	delta[0]=_readPureDataFloat(dataPointer,0,0); /* pure data is the data that is not used to identify a command */
